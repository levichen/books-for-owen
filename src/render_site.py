...
def inject_autorefresh(page):
    ...
    若任一紀錄頁的離線佇列還有東西（busy），這次不重載，避免打斷送出中的請求。"""
    js = (
        "<script>/*#AR#" + BUILD + "*/(function(){var B='" + BUILD + "';"
        "function busy(){try{var ks=['owen-reading-queue-v1','owen-drink-queue-v1','owen-jump-queue-v1','owen-wq-pqueue-v1'];"
        "for(var i=0;i<ks.length;i++){var v=JSON.parse(localStorage.getItem(ks[i])||'null');"
        "if(v&&(Array.isArray(v)?v.length:Object.keys(v).length))return true;}}catch(e){}return false;}"
        ...
        "if(!sessionStorage.getItem(k)&&!busy()){sessionStorage.setItem(k,'1');location.reload();}}"
...
if(doneBtn)doneBtn.onclick=async()=>{
 ...
 try{
  ...
  const resp=await (await fetch(LOG_API,{method:'POST',body:JSON.stringify({action:'add',date:today,title:BOOK_TITLE,id:'r'+Date.now().toString(36)+Math.random().toString(36).slice(2,10)})})).json();
