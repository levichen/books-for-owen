...
.pg{{display:none;width:min(96vw,920px)}}
.pg.on{{display:block}}
...
footer{{display:flex;align-items:center;justify-content:center;gap:18px;padding:10px 0 16px}}
button{{width:58px;height:58px;border:none;border-radius:50%;background:#E4574C;color:#fff;
       font-size:24px;cursor:pointer;box-shadow:0 4px 12px rgba(228,87,76,.35);display:flex;align-items:center;justify-content:center}}
button:disabled{{background:#E5D9C9;box-shadow:none;cursor:default}}
...
<header><a href="../../index.html">&larr; 書架 Bookshelf</a><div class="ttl">{BOOK_TITLE_EN}</div><div style="width:110px"></div></header>
<div id="stage">{''.join(secs)}</div>
<footer><button id="prev" aria-label="previous page">&#8249;</button><div id="cnt"></div><button id="next" aria-label="next page">&#8250;</button></footer>
