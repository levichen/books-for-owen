# -*- coding: utf-8 -*-
"""跳繩次數頁（site/jump-log/）：記錄 Owen 每天跳繩次數，累積 10000 次換禮物（循環里程碑）。

後端：與閱讀紀錄/牛奶點數共用同一個 Apps Script API，走通用計數器 action
（counter_add/counter_del、GET ?mode=counter&sheet=jumps），資料在試算表 jumps 工作表。
後端未更新前操作留在離線佇列，不會誤寫其他紀錄。
"""

from reading_log import API_URL

GOAL = 10000  # 每累積 10000 次換一個禮物
SHEET = "jumps"

STYLE = """
@font-face { font-family:'Huninn'; src:url('../assets/huninn.woff2') format('woff2'); font-display:swap; }
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Huninn',system-ui,sans-serif;background:#F1F8F1;min-height:100vh;color:#33513A}
.wrap{max-width:760px;margin:0 auto;padding:26px 16px 60px}
h1{font-size:clamp(24px,5vw,34px);text-align:center}
.sub{color:#6E8F74;text-align:center;margin:6px 0 6px;font-size:14px}
.back{display:inline-block;color:#6E8F74;text-decoration:none;font-size:14px;margin-bottom:6px}
.card{background:#fff;border-radius:22px;box-shadow:0 8px 26px rgba(51,81,58,.10);padding:18px;margin-bottom:18px}

/* --- 同步狀態 --- */
.status{display:flex;justify-content:center;margin-bottom:14px}
.status span{font-size:12px;color:#6E8F74;background:#E3F0E4;border-radius:999px;padding:4px 12px}
.status span.ok{background:#E4F2E4;color:#4C7A4C}
.status span.bad{background:#FDEAE0;color:#B0563F}

/* --- 進度區 --- */
.progress{display:flex;align-items:center;gap:18px;flex-wrap:wrap;justify-content:center}
.donut{position:relative;width:170px;height:170px;flex:0 0 auto}
.donut svg{width:100%;height:100%;transform:rotate(-90deg)}
.donut .center{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center}
.donut .num{font-size:32px;line-height:1}
.donut .pct{font-size:12px;color:#6E8F74;margin-top:4px}
.gift{flex:1 1 220px;min-width:220px}
.gift .big{font-size:19px}
.gift .left{color:#6E8F74;font-size:14px;margin-top:6px}
.gift .done{color:#43A047;font-size:17px;margin-top:6px}
.milestones{display:flex;gap:6px;margin-top:12px;flex-wrap:wrap}
.mile{font-size:12px;color:#6E8F74;background:#F1F8F1;border-radius:999px;padding:4px 10px}
.mile.hit{background:#43A047;color:#fff}

/* --- 輸入 --- */
.form-row{display:flex;gap:8px;flex-wrap:wrap}
input[type=date],input[type=number]{font-family:inherit;font-size:15px;color:#33513A;
  border:2px solid #CDE5CF;border-radius:12px;padding:10px 12px;background:#FCFFFC;outline:none}
input:focus{border-color:#43A047}
input[type=date]{flex:0 0 auto}
input[type=number]{flex:1 1 140px;min-width:0}
button{font-family:inherit;font-size:15px;border:none;cursor:pointer}
.add-btn{background:#43A047;color:#fff;padding:10px 22px;border-radius:999px}
.add-btn:active{transform:scale(.97)}
.hint{color:#9DB8A1;font-size:12px;margin-top:8px}
.err{color:#B0563F;font-size:13px;margin-top:8px;display:none}

/* --- 行事曆 --- */
.cal-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}
.cal-head .m{font-size:18px}
.nav-btn{background:#F1F8F1;color:#33513A;width:36px;height:36px;border-radius:50%;font-size:16px}
.cal{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.dow{text-align:center;color:#9DB8A1;font-size:12px;padding:4px 0}
.day{position:relative;aspect-ratio:1;border-radius:10px;background:#FCFFFC;border:2px solid transparent;
  display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;font-size:14px}
.day.blank{background:transparent;cursor:default}
.day.today{border-color:#F0B429}
.day.sel{border-color:#43A047}
.day .cnt{position:absolute;right:2px;top:2px;background:#43A047;color:#fff;font-size:9px;
  min-width:16px;height:15px;border-radius:999px;display:flex;align-items:center;justify-content:center;padding:0 3px}
.day.has{background:#E3F0E4}

/* --- 當日清單 --- */
.day-title{font-size:15px;margin-bottom:8px}
.entry{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px dashed #E3F0E4}
.entry:last-child{border-bottom:none}
.entry .bk{flex:1;font-size:16px}
.entry .pend{font-size:12px;color:#9DB8A1}
.del{background:#F1F8F1;color:#6E8F74;padding:6px 12px;font-size:13px;border-radius:999px}
.empty{color:#9DB8A1;font-size:14px}
footer{text-align:center;color:#9DB8A1;font-size:13px;margin-top:30px}
"""

SCRIPT = """
'use strict';
const API = '__API__';
const GOAL = __GOAL__;
const SHEET = '__SHEET__';
const CACHE_KEY = 'owen-jump-cache-v1';
const QUEUE_KEY = 'owen-jump-queue-v1';

/* ---------- 本機儲存 ---------- */
function lsGet(key, fallback) {
  try {
    const raw = localStorage.getItem(key);
    if (!raw) return fallback;
    const v = JSON.parse(raw);
    return Array.isArray(v) ? v : fallback;
  } catch (err) { console.error('read', key, err); return fallback; }
}
function lsSet(key, val) {
  try { localStorage.setItem(key, JSON.stringify(val)); }
  catch (err) { console.error('write', key, err); }
}

/* ---------- 狀態 ---------- */
let serverEntries = lsGet(CACHE_KEY, []);   // [{id, date, value}]
let queue = lsGet(QUEUE_KEY, []);           // [{action:'counter_add',date,value,tmp} | {action:'counter_del',id}]
let flushing = false;
const todayStr = localDateStr(new Date());
let view = { y: +todayStr.slice(0, 4), m: +todayStr.slice(5, 7) - 1 };
let selected = todayStr;

function localDateStr(d) {
  return d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
}
function newTmpId() { return 'tmp-' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7); }

function currentEntries() {
  let out = serverEntries.slice();
  for (const op of queue) {
    if (op.action === 'counter_add') out = out.concat([{ id: op.tmp, date: op.date, value: op.value, pending: true }]);
    if (op.action === 'counter_del') out = out.filter(e => e.id !== op.id);
  }
  return out;
}
function byDate(date) { return currentEntries().filter(e => e.date === date); }
function sumOf(list) { return list.reduce((s, e) => s + (parseInt(e.value, 10) || 0), 0); }

/* 表現扣分（penalty 工作表，三本共用）：total 顯示時扣除 */
let penalty = 0;
try { penalty = parseInt(localStorage.getItem('owen-penalty-cache') || '0', 10) || 0; } catch (e) {}
function fetchPenalty() {
  fetch(API + '?mode=counter&sheet=penalty', { cache: 'no-store' })
    .then(r => r.json())
    .then(j => {
      if (j && j.ok && Array.isArray(j.items)) {
        penalty = j.items.reduce((s, e) => s + (parseInt(e.value, 10) || 0), 0);
        try { localStorage.setItem('owen-penalty-cache', String(penalty)); } catch (e) {}
        renderProgress();
      }
    }).catch(() => {});
}

/* ---------- 同步 ---------- */
function apiFetch(opts) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 20000);
  const url = (opts && opts.method === 'POST') ? API : API + '?mode=counter&sheet=' + SHEET;
  return fetch(url, Object.assign({ signal: ctrl.signal }, opts))
    .then(r => { if (!r.ok) throw new Error('http ' + r.status); return r.json(); })
    .finally(() => clearTimeout(timer));
}
async function flush() {
  if (flushing) return;
  flushing = true;
  setStatus('sync');
  try {
    while (queue.length) {
      const op = queue[0];
      const body = op.action === 'counter_add'
        ? { action: 'counter_add', sheet: SHEET, date: op.date, value: op.value }
        : { action: 'counter_del', sheet: SHEET, id: op.id };
      const resp = await apiFetch({ method: 'POST', body: JSON.stringify(body) });
      if (!resp.ok) {
        if (resp.error === 'unknown action') { setStatus('stale'); return; }  // 後端未更新：保留佇列
        if (resp.error === 'bad input' || resp.error === 'no sheet') { queue = queue.slice(1); lsSet(QUEUE_KEY, queue); continue; }
        throw new Error(resp.error || 'server error');
      }
      queue = queue.slice(1);
      lsSet(QUEUE_KEY, queue);
      serverEntries = resp.items || [];
      lsSet(CACHE_KEY, serverEntries);
      renderAll();
    }
    const resp = await apiFetch({ method: 'GET' });
    if (resp.ok && Array.isArray(resp.items)) {
      serverEntries = resp.items;
      lsSet(CACHE_KEY, serverEntries);
      setStatus('ok');
    } else if (resp.ok) {
      setStatus('stale');
    }
  } catch (err) {
    console.error('sync failed', err);
    setStatus(queue.length ? 'offline' : 'error');
  } finally {
    flushing = false;
    renderAll();
  }
}

/* ---------- 操作 ---------- */
function addJump(date, value) {
  queue = queue.concat([{ action: 'counter_add', date, value, tmp: newTmpId() }]);
  lsSet(QUEUE_KEY, queue);
  renderAll();
  flush();
}
function delJump(id) {
  if (String(id).startsWith('tmp-')) {
    queue = queue.filter(op => !(op.action === 'counter_add' && op.tmp === id));
  } else {
    queue = queue.concat([{ action: 'counter_del', id }]);
  }
  lsSet(QUEUE_KEY, queue);
  renderAll();
  flush();
}

/* ---------- 畫面 ---------- */
function setStatus(state) {
  const el = document.getElementById('sync-status');
  const pend = queue.length;
  if (state === 'sync') { el.className = ''; el.textContent = '同步中…'; }
  else if (state === 'ok') { el.className = 'ok'; el.textContent = '已同步（多裝置共用）'; }
  else if (state === 'offline') { el.className = 'bad'; el.textContent = '離線中，' + pend + ' 筆待同步（連線後自動補送）'; }
  else if (state === 'stale') { el.className = 'bad'; el.textContent = '雲端後端待更新' + (pend ? '，' + pend + ' 筆已保留待補送' : '') + '（請爸爸更新 Apps Script）'; }
  else { el.className = 'bad'; el.textContent = '連不上雲端，先顯示上次資料'; }
}
function fmt(n) { return n.toLocaleString('en-US'); }
function renderProgress() {
  const total = Math.max(0, sumOf(currentEntries()) - penalty);
  const cycles = Math.floor(total / GOAL);
  const justHit = total > 0 && total % GOAL === 0;
  const inCycle = justHit ? GOAL : total % GOAL;
  const pct = Math.round(inCycle / GOAL * 100);
  const C = 2 * Math.PI * 70;
  const arc = document.getElementById('arc');
  arc.setAttribute('stroke-dasharray', (pct / 100 * C) + ' ' + C);
  arc.style.display = pct === 0 ? 'none' : '';
  document.getElementById('total').textContent = fmt(total);
  document.getElementById('pct').textContent = '本輪 ' + fmt(inCycle) + ' / ' + fmt(GOAL);
  const gift = document.getElementById('gift-msg');
  let html = '';
  if (cycles > 0) {
    html += '<div class="done">' + '\\u{1F381}'.repeat(Math.min(cycles, 8)) +
            ' 已達成 <b>' + cycles + '</b> 次換禮物！</div>';
  }
  if (justHit) {
    html += '<div class="left">第 ' + cycles + ' 個 ' + fmt(GOAL) + ' 次達成！先去換禮物，下一輪繼續 &#127881;</div>';
  } else {
    html += '<div class="left">再跳 <b>' + fmt(GOAL - total % GOAL) + '</b> 次換第 ' + (cycles + 1) + ' 個禮物 &#127873;</div>';
  }
  if (penalty > 0) {
    html += '<div class="left" style="color:#B0563F">&#9888;&#65039; 表現扣分 &minus;' + penalty + '</div>';
  }
  gift.innerHTML = html;
  const base = justHit ? (cycles - 1) * GOAL : cycles * GOAL;
  const q = GOAL / 4;
  const miles = [base + q, base + 2 * q, base + 3 * q, base + GOAL];
  document.getElementById('miles').innerHTML = miles.map(m =>
    '<span class="mile' + (total >= m ? ' hit' : '') + '">' + fmt(m) + '</span>').join('');
}
function renderCalendar() {
  const { y, m } = view;
  document.getElementById('cal-month').textContent = y + ' 年 ' + (m + 1) + ' 月';
  const first = new Date(y, m, 1).getDay();
  const days = new Date(y, m + 1, 0).getDate();
  const cells = [];
  for (let i = 0; i < first; i++) cells.push('<div class="day blank"></div>');
  for (let d = 1; d <= days; d++) {
    const ds = y + '-' + String(m + 1).padStart(2, '0') + '-' + String(d).padStart(2, '0');
    const n = sumOf(byDate(ds));
    const cls = ['day', n ? 'has' : '', ds === todayStr ? 'today' : '', ds === selected ? 'sel' : ''].filter(Boolean).join(' ');
    cells.push('<div class="' + cls + '" data-date="' + ds + '">' + d + (n ? '<span class="cnt">' + n + '</span>' : '') + '</div>');
  }
  document.getElementById('cal-grid').innerHTML =
    ['日','一','二','三','四','五','六'].map(w => '<div class="dow">' + w + '</div>').join('') + cells.join('');
}
function renderDay() {
  const list = byDate(selected);
  document.getElementById('day-title').textContent = selected + ' 跳了 ' + fmt(sumOf(list)) + ' 次';
  const box = document.getElementById('day-list');
  if (!list.length) { box.innerHTML = '<div class="empty">這天還沒有紀錄</div>'; return; }
  box.innerHTML = list.map(e =>
    '<div class="entry"><span class="bk">&#129336; ' + fmt(parseInt(e.value, 10) || 0) + ' 次</span>' +
    (e.pending ? '<span class="pend">待同步</span>' : '') +
    '<button class="del" data-id="' + e.id + '">刪除</button></div>').join('');
}
function renderAll() { renderProgress(); renderCalendar(); renderDay(); }
function showError(msg) { const el = document.getElementById('err'); el.textContent = msg; el.style.display = 'block'; }
function clearError() { document.getElementById('err').style.display = 'none'; }

/* ---------- 事件 ---------- */
document.getElementById('add-form').addEventListener('submit', ev => {
  ev.preventDefault(); clearError();
  const date = document.getElementById('in-date').value;
  const v = parseInt(document.getElementById('in-count').value, 10);
  if (!/^\\d{4}-\\d{2}-\\d{2}$/.test(date)) { showError('請選擇日期'); return; }
  if (!Number.isInteger(v) || v < 1 || v > 9999) { showError('請填 1〜9999 的次數'); return; }
  addJump(date, v);
  selected = date;
  view = { y: +date.slice(0, 4), m: +date.slice(5, 7) - 1 };
  document.getElementById('in-count').value = '';
  renderAll();
});
document.getElementById('cal-grid').addEventListener('click', ev => {
  const cell = ev.target.closest('.day');
  if (!cell || !cell.dataset.date) return;
  selected = cell.dataset.date;
  renderCalendar(); renderDay();
});
document.getElementById('day-list').addEventListener('click', ev => {
  const btn = ev.target.closest('.del');
  if (!btn) return;
  delJump(btn.dataset.id);
});
document.getElementById('prev-m').addEventListener('click', () => {
  view = view.m === 0 ? { y: view.y - 1, m: 11 } : { y: view.y, m: view.m - 1 };
  renderCalendar();
});
document.getElementById('next-m').addEventListener('click', () => {
  view = view.m === 11 ? { y: view.y + 1, m: 0 } : { y: view.y, m: view.m + 1 };
  renderCalendar();
});
window.addEventListener('online', flush);
document.addEventListener('visibilitychange', () => { if (!document.hidden) flush(); });

document.getElementById('in-date').value = todayStr;
renderAll();
flush();
fetchPenalty();
"""


def jump_log_html():
    script = (SCRIPT.replace("__GOAL__", str(GOAL))
                    .replace("__API__", API_URL)
                    .replace("__SHEET__", SHEET))
    return f"""<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Owen 的跳繩次數</title>
<meta name="robots" content="noindex">
<style>{STYLE}</style></head><body><div class="wrap">
<a class="back" href="../">&larr; 回書架</a>
<h1>&#129336; Owen 的跳繩次數</h1>
<div class="sub">每天跳的都記下來，每滿 10,000 次換一個禮物 &#127873;</div>
<div class="status"><span id="sync-status">同步中&hellip;</span></div>

<div class="card">
  <div class="progress">
    <div class="donut">
      <svg viewBox="0 0 170 170">
        <circle cx="85" cy="85" r="70" fill="none" stroke="#DDEDDE" stroke-width="18"/>
        <circle id="arc" cx="85" cy="85" r="70" fill="none" stroke="#43A047" stroke-width="18"
                stroke-linecap="round" stroke-dasharray="0 440"/>
      </svg>
      <div class="center"><div class="num"><span id="total">0</span></div><div class="pct"><span id="pct">本輪 0 / 10,000</span></div></div>
    </div>
    <div class="gift">
      <div class="big">禮物進度</div>
      <div id="gift-msg"></div>
      <div class="milestones" id="miles"></div>
    </div>
  </div>
</div>

<div class="card">
  <form id="add-form" class="form-row">
    <input type="date" id="in-date" required>
    <input type="number" id="in-count" placeholder="今天跳了幾次？" min="1" max="9999" required>
    <button type="submit" class="add-btn">新增</button>
  </form>
  <div class="err" id="err"></div>
  <div class="hint">一天可以記很多筆（早上跳一輪、晚上再跳一輪都分開記）。所有裝置共用同一份雲端紀錄，離線也能記。</div>
</div>

<div class="card">
  <div class="cal-head">
    <button class="nav-btn" id="prev-m">&#8249;</button>
    <div class="m" id="cal-month"></div>
    <button class="nav-btn" id="next-m">&#8250;</button>
  </div>
  <div class="cal" id="cal-grid"></div>
</div>

<div class="card">
  <div class="day-title" id="day-title"></div>
  <div id="day-list"></div>
</div>

<footer>made with &hearts; by Daddy &amp; Claude</footer>
</div><script>{script}</script></body></html>"""
