...
.zh-hint{font-size:20px;color:#3B3352;margin-bottom:10px}
...
.wrongs .w small{color:#7E749A;font-size:14px}
...
.card .zh{font-size:24px;color:#7E749A}
...
function renderHome() {
  ...
  $('start').textContent = p.length < 2 ? '單字不夠' : '開始小考 ▶';
}
...
function renderSpell(w) {
  $('qtype').textContent = '聽音拼字';
  $('zhHint').textContent = w.zh;
...
function answer(correct) {
  ...
  fb.innerHTML = correct ? '&#10004; 答對了！' : '&#10006; 是 <b>' + w.word + '</b>';
  if (correct) sfx.ok(); else sfx.ng();
  setTimeout(() => speak(w.word), 300);
  $('next').textContent = quiz.i + 1 >= quiz.items.length ? '看成績 ★' : '下一題 ▶';
...
function finish() {
  ...
  $('rstat').textContent = right + ' / ' + n + ' 題答對';
  $('rrec').textContent = score === 100 ? '🏆 滿分！全部都會了！' : newAll && best.plays > 1 ? '🏆 歷代最佳紀錄！' : newToday && prevToday !== null ? '⭐ 今天的新紀錄！' : '';
  $('wrongs').innerHTML = quiz.wrongs.length
    ? '<div class="stat">這些字明天會再考你一次：</div>' + quiz.wrongs.map(w =>
        '<div class="w"><button data-say="' + w.word.replace(/"/g, '&quot;') + '">&#128264;</button><span>' + w.word + '</span><small>' + w.zh + '</small></div>').join('')
    : '';
...
function renderCard() {
  ...
  $('cardZh').textContent = card.flipped ? w.zh : '（點一下看意思）';
...
<div id="home">
  ...
  <div class="panel">
    ...
    <div><button class="bigbtn alt" id="cards-btn">先翻單字卡 &#128264;</button></div>
...
<div id="quiz" class="hidden">
  ...
  <div class="panel">
    ...
    <div id="spellQ" class="hidden">
      ...
      <div class="tools"><button class="tool" id="undo">&#9003; 退一格</button></div>
    </div>
...
<div id="result" class="hidden">
  <div class="panel">
    ...
    <button class="bigbtn" id="again">再考一次 &#9654;</button>
    <div><button class="bigbtn alt" id="home2">回首頁</button></div>
  </div>
</div>
...
<div id="cards" class="hidden">
  ...
  <div class="card" id="card">
    ...
    <button class="sp" id="cardSay">&#128264; 再唸一次</button>
  </div>
  <div class="cardnav">
    <button class="bigbtn alt" id="cardPrev">&#9664; 上一張</button>
    <button class="bigbtn alt" id="cardNext">下一張 &#9654;</button>
  </div>
  <div><button class="tool" id="cardHome" style="margin-top:14px">回首頁</button></div>
</div>
