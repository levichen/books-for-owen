...
# 學校單字表（照片轉錄）——試算表沒資料時的備援。U1（09-05、09-12）、U2（09-12、09-21）、U3–U9（2026-09-21）。
# 順序＝單元＋課本頁面順序（學習順序依此）。跨單元重複的字只留第一次出現（favorite/live/sleep/watch/grow/hot/cold…）
SEED_WORDS = [
    ('family', '家人', 'U1'),
    ('grandmother', '奶奶；外婆', 'U1'),
    ('grandfather', '爺爺；外公', 'U1'),
    ('father', '爸爸', 'U1'),
    ('mother', '媽媽', 'U1'),
    ('brother', '哥哥；弟弟', 'U1'),
    ('sister', '姊姊；妹妹', 'U1'),
    ...
    ('we', '我們', 'U1'),
    ('they', '他們', 'U1'),
    ('are', '是（與 you, we, they 連用）', 'U1'),
    ('he', '他', 'U1'),
    ('she', '她', 'U1'),
    ('it', '它；牠', 'U1'),
    ('is', '是（與 he, she, it 連用）', 'U1'),
    ('uncle', '伯父；叔叔；姑丈；姨丈；舅舅', 'U1'),
    ('aunt', '伯母；嬸嬸；姑姑；阿姨；舅媽', 'U1'),
    ('daughter', '女兒', 'U1'),
    ('parents', '父母親', 'U1'),
    ('son', '兒子', 'U1'),
    ('cousin', '堂（表）兄弟姊妹', 'U1'),
    ...
    ('a lot of', '很多的', 'U4'),
    ('in the world', '全世界', 'U4'),
    ('informational text', '記敘文', 'U4'),
    ('art', '美術、藝術', 'U4'),
    ('primary', '基本的', 'U4'),
    ('secondary', '次要的', 'U4'),
    ('color wheel', '色環；色輪', 'U4'),
    ('next to', '在……旁邊', 'U4'),
    ('paint', '繪畫', 'U4'),
    ('finish', '完成', 'U4'),
    ('lots of', '很多的', 'U4'),
    ...
    ('fact', '事實；真相', 'U5'),
    ('food', '食物', 'U5'),
    ('all winter long', '整個冬天', 'U5'),
    ('get big', '長大', 'U5'),
    ('Where is ...?', '……在哪裡？（用於單數名詞）', 'U5'),
    ('Where are ...?', '……在哪裡？（用於複數名詞）', 'U5'),
    ('woods', '樹林', 'U5'),
    ('field', '田地；原野', 'U5'),
    ('pond', '池塘', 'U5'),
    ('squirrel', '松鼠', 'U5'),
    ...
    ('take care of', '照顧……', 'U6'),
    ('miss', '想念', 'U6'),
    ('take a nap', '睡午覺；小睡一下', 'U6'),
    ('put out', '拿出；提供；準備', 'U6'),
    ('tired', '疲勞的；累的', 'U6'),
    ('watch', '觀看', 'U6'),
    ('Indonesia', '印尼', 'U6'),
    ('realistic fiction', '寫實小說', 'U6'),
    ('about', '關於', 'U6'),
    ('be called...', '被稱為……', 'U6'),
    ("don't have", '沒有（用於第一、二人稱）', 'U6'),
    ('alone', '獨自一人', 'U6'),
    ('bottle', '瓶子', 'U6'),
    ('fruit', '水果', 'U6'),
    ('flower', '花', 'U6'),
    ...
    ('cover', '覆蓋', 'U7'),
    ('in the fall', '在秋天', 'U7'),
    ('simple present', '現在簡單式', 'U7'),
    ('get', '變成', 'U7'),
    ("doesn't get", '沒有變成（用於第三人稱單數）', 'U7'),
    ("doesn't snow", '沒有下雪（用於第三人稱單數）', 'U7'),
    ('does not', "沒有（= doesn't，用於第三人稱單數）", 'U7'),
    ('weather', '天氣', 'U7'),
    ('cloudy', '多雲的', 'U7'),
    ('sunny', '晴朗的', 'U7'),
    ('windy', '有風的', 'U7'),
    ...
    ('rainy', '多雨的；下雨的', 'U7'),
    ('in the snow', '在雪地裡', 'U7'),
    ('compound noun', '複合名詞', 'U7'),
    ('doorbell', '門鈴', 'U7'),
    ('bluebird', '藍知更鳥', 'U7'),
    ('build a snowman', '做雪人', 'U8'),
    ('build a tree house', '蓋（建造）樹屋', 'U8'),
    ('make a swing', '做鞦韆', 'U8'),
    ('make apple pie', '做蘋果派', 'U8'),
    ('bring', '拿；帶來', 'U8'),
    ('make', '製作', 'U8'),
    ('on the beach', '在海邊、在海灘上', 'U8'),
    ('insect', '昆蟲', 'U8'),
    ('in the field', '在原野', 'U8'),
    ('busy', '忙碌的', 'U8'),
    ('blossom', '花', 'U8'),
    ...
    ('melt', '融化', 'U8'),
    ('really', '真地', 'U8'),
    ("don't", '不（= do not，用於第一、二人稱）', 'U8'),
    ("don't make", '不製作（用於第一、二人稱）', 'U8'),
    ('build', '建造', 'U8'),
    ("don't build", '不建造（用於第一、二人稱）', 'U8'),
    ('do', '助動詞（用於第一、二人稱）', 'U8'),
    ('ride a bicycle', '騎腳踏車', 'U8'),
    ('go to the beach', '去海邊', 'U8'),
    ('eat ice cream', '吃冰淇淋', 'U8'),
    ('drink hot chocolate', '喝熱可可', 'U8'),
    ('fly a kite', '放風箏', 'U8'),
    ('plant flowers', '種花', 'U8'),
    ('comma', '逗號', 'U8'),
    ('tweet', '吱吱叫', 'U8'),
    ('Yum!', '真好吃！', 'U8'),
    ('What a mess!', '真是一團糟', 'U8'),
    ('journal', '日誌', 'U8'),
    ('number', '數字', 'U9'),
    ('plus sign', '加號', 'U9'),
    ('equals sign', '等號', 'U9'),
    ...
    ('join', '加入；將……加在一起', 'U9'),
    ('group', '組、群', 'U9'),
    ('the same amount', '相同的數量', 'U9'),
    ('work out', '解決', 'U9'),
    ('How many...?', '多少……？', 'U9'),
    ('have', '有（用於 I, you, we, they）', 'U9'),
    ('pen', '筆', 'U9'),
    ('pencil', '鉛筆', 'U9'),
    ('eraser', '橡皮擦', 'U9'),
    ('ruler', '尺', 'U9'),
    ('backpack', '背包', 'U9'),
    ('notebook', '筆記本', 'U9'),
    ('How old are you?', '你幾歲？', 'U9'),
    ('How about you?', '那你呢？', 'U9'),
    ('eleven', '十一', 'U9'),
    ('twelve', '十二', 'U9'),
    ('thirteen', '十三', 'U9'),
    ('fourteen', '十四', 'U9'),
...
.boxes{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:14px}
.box{background:#F7F5FC;border-radius:14px;padding:10px 2px 8px}
.box .ic{font-size:22px;line-height:1}
.box .n{font-size:26px;line-height:1.1;color:#3B3352;margin:4px 0 2px}
.box .lb{font-size:15px;color:#7E749A;line-height:1.3}
.box .lb .zc{margin-right:.08em}
@media (max-width:480px){.boxes{grid-template-columns:repeat(2,1fr)}.box{display:flex;align-items:center;justify-content:center;gap:10px;padding:10px 8px}.box .n{margin:0}}
...
.plan{font-size:18px;line-height:1.5;margin:6px 0 4px;display:flex;flex-direction:column;gap:8px;align-items:center}
.plan .pl{display:flex;align-items:center;gap:8px}
.plan .pl .num{background:#6C4DD6;color:#fff;border-radius:999px;min-width:34px;padding:2px 10px;font-size:18px}
...
function normWords(list) {
  ...
  return sortByUnit(out);
}
/* 進度鍵：小寫、去掉標點（Watch out! → watch out），符合雲端 kv key 規則 */
function keyOf(word) { return String(word).toLowerCase().replace(/[^a-z0-9' -]/g, '').trim(); }
/* 學習順序＝先單元（U1→U2→…，取數字）、同單元照表格順序（stable sort） */
function unitNo(u) { const m = String(u || '').match(/\d+/); return m ? parseInt(m[0], 10) : 9999; }
function sortByUnit(list) { return list.map((w, i) => [w, i]).sort((a, b) => (unitNo(a[0].unit) - unitNo(b[0].unit)) || (a[1] - b[1])).map(x => x[0]); }
...
function unitsOf() { return Array.from(new Set(words.map(w => w.unit).filter(Boolean))).sort((a, b) => unitNo(a) - unitNo(b)); }
...
function stOf(w) { return state[keyOf(w.word)] || null; }
...
function introduce(w) {
  const k = keyOf(w.word), old = state[k] || { box: 0, seen: 0, wrong: 0 };
  if (old.intro) return;
  saveState(k, Object.assign({}, old, { intro: todayStr(), last: todayStr() }));
}
function mark(w, correct) {
  const k = keyOf(w.word), old = state[k] || { box: 0, seen: 0, wrong: 0, intro: todayStr() };
...
function renderHome() {
  ...
  const box = (ic, n, lb) => '<div class="box"><div class="ic">' + ic + '</div><div class="n">' + n + '</div><div class="lb">' + ruby(lb) + '</div></div>';
  $('boxes').innerHTML = box('&#128218;', nNew, '還沒學') + box('&#127793;', n0, '不熟') + box('&#127807;', n1, '快會了') + box('&#127794;', n2, '已經會');
  ...
  const pline = (ic, lb, n, unitLb) => '<div class="pl"><span>' + ic + ' ' + ruby(lb) + '</span><span class="num">' + n + '</span><span>' + ruby(unitLb) + '</span></div>';
  $('plan').innerHTML =
    (pl.fresh.length ? pline('&#128218;', '學新字', pl.fresh.length, '個') : '') +
    (pl.weak.length ? pline('&#127793;', '複習不熟', pl.weak.length, '個') : '') +
    (pl.quizN >= 2 ? pline('&#128221;', '小考', pl.quizN, '題') : '');
